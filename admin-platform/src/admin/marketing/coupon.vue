...
const formatAmount = (val: any): string => { const num = Number(val); return isNaN(num) ? '0.00' : num.toFixed(2) }
...
const memberLevelLabel = (level: number): string => { const map: Record<number,string>={1:'普通会员',2:'银卡会员',3:'金卡会员',4:'钻石会员'}; return map[level]||'不限' }

interface CouponItem {
  id: number; title: string; type: string; discountAmount: number; minConsume: number
  totalCount: number; receivedCount: number; perUserLimit: number
  validStart: string; validEnd: string; exchangePoints: number; memberLevelRequired: number; status: string
  validityType?: number; validityDays?: number
}

const couponSearch = reactive({ keyword: '', type: '', status: '' })
const couponPage = reactive({ page: 1, pageSize: 10 })
const couponTotal = ref(0)
const couponList = ref<CouponItem[]>([])
const couponTotalPages = computed(() => { const t = Math.ceil(couponTotal.value / couponPage.pageSize); return t > 0 ? t : 1 })

const couponDialog = reactive({
  visible: false, isEdit: false, editId: null as number | null,
  form: { title: '', type: 'FIXED', discountAmount: 0, minConsume: 0.01, totalCount: 0, perUserLimit: 1, validStart: '', validEnd: '', validityType: 1, validityDays: 7, exchangePoints: 0, memberLevelRequired: 1 }
})

const loadCouponList = async () => {
  const params: any = { page: couponPage.page, pageSize: couponPage.pageSize }
  ...
  try {
    ...
    if (res.code === 0) {
      const list = (res.data.list || res.data || []).map((item: any) => ({ ...item, type: (item.type === 1 || item.type === 'FIXED') ? 'FIXED' : 'FULL_REDUCTION', status: (item.status === 1 || item.status === 'ACTIVE') ? 'ACTIVE' : 'DISABLED' }))
      couponList.value = list; couponTotal.value = res.data.total || list.length
    }
...
const openCouponDialog = (item?: CouponItem) => {
  if (item) {
    couponDialog.isEdit = true; couponDialog.editId = item.id
    couponDialog.form = { title: item.title, type: item.type, discountAmount: item.discountAmount, minConsume: item.minConsume, totalCount: item.totalCount, perUserLimit: item.perUserLimit, validStart: item.validStart, validEnd: item.validEnd, validityType: item.validityType || 1, validityDays: item.validityDays || 7, exchangePoints: item.exchangePoints || 0, memberLevelRequired: item.memberLevelRequired || 1 }
  } else {
    couponDialog.isEdit = false; couponDialog.editId = null
    couponDialog.form = { title: '', type: 'FIXED', discountAmount: 0, minConsume: 0.01, totalCount: 0, perUserLimit: 1, validStart: '', validEnd: '', validityType: 1, validityDays: 7, exchangePoints: 0, memberLevelRequired: 1 }
  }
  couponDialog.visible = true
}

const handleSaveCoupon = async () => {
  if (couponDialog.form.validityType === 1 && couponDialog.form.validStart && couponDialog.form.validEnd) { if (new Date(couponDialog.form.validEnd) <= new Date(couponDialog.form.validStart)) { alert('结束时间必须晚于开始时间'); return } }
  const data: any = { ...couponDialog.form }; data.discountAmount = Number(data.discountAmount) || 0; data.minConsume = Number(data.minConsume) || 0.01; data.totalCount = Number(data.totalCount) || 0; data.perUserLimit = Number(data.perUserLimit) || 1; data.exchangePoints = Number(data.exchangePoints) || 0
  try { if (couponDialog.isEdit && couponDialog.editId !== null) await updateCoupon(couponDialog.editId, data); else await createCoupon(data); couponDialog.visible = false; loadCouponList() } catch (e: any) { alert(e?.response?.data?.msg || '保存失败') }
}

const handleDeleteCoupon = async (item: CouponItem) => { if (confirm(`确定删除优惠券"${item.title}"吗？`)) { try { await deleteCoupon(item.id); loadCouponList() } catch (e: any) { alert(e?.response?.data?.msg || '删除失败') } } }
const handleDisableCoupon = async (item: CouponItem) => { if (confirm(`确定禁用优惠券"${item.title}"吗？`)) { try { await disableCoupon(item.id); loadCouponList() } catch (e: any) { alert(e?.response?.data?.msg || '禁用失败') } } }
const handleEnableCoupon = async (item: CouponItem) => { try { await enableCoupon(item.id); loadCouponList() } catch (e: any) { alert(e?.response?.data?.msg || '启用失败') } }
...
.search-btn { padding: 8px 20px; background: #165DFF; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px; }
...
.page-btn { padding: 8px 16px; border: 1px solid #dcdcdc; background: white; border-radius: 4px; cursor: pointer; font-size: 14px; }
