  ...
  <div class="distributor-audit-page">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id">
            ...
            <td>
              ...
              <button v-if="item.auditStatus === 0" class="btn-approve" @click="audit(item, 1)">通过</button>
              <button v-if="item.auditStatus === 0" class="btn-reject" @click="openReject(item)">拒绝</button>
              <span v-if="item.auditStatus !== 0 && item.auditRemark" class="remark-text">{{ item.auditRemark }}</span>
            </td>
          </tr>
        </tbody>
      </table>
      <div v-else class="empty-state"><span class="empty-text">暂无分销员申请</span></div>
    </div>
    ...
    <div v-if="detailVisible" class="modal-mask" @click.self="detailVisible = false">
      <div class="modal-box modal-wide">
        ...
        <div class="detail-grid">
          <div class="detail-item"><label>申请ID</label><span>{{ detailItem.id }}</span></div>
          <div class="detail-item"><label>用户ID</label><span>{{ detailItem.userId }}</span></div>
          <div class="detail-item"><label>用户名</label><span>{{ detailItem.nickname || '-' }}</span></div>
          <div class="detail-item"><label>真实姓名</label><span>{{ detailItem.realName }}</span></div>
          <div class="detail-item"><label>手机号</label><span>{{ detailItem.phone || '-' }}</span></div>
          <div class="detail-item"><label>身份证号</label><span>{{ detailItem.idCardNo }}</span></div>
          <div class="detail-item"><label>开户银行</label><span>{{ detailItem.bankName }}</span></div>
          <div class="detail-item"><label>银行卡号</label><span>{{ detailItem.bankCardNo }}</span></div>
          <div class="detail-item"><label>开户人</label><span>{{ detailItem.bankAccountName }}</span></div>
          <div class="detail-item"><label>申请时间</label><span>{{ detailItem.applyTime || '-' }}</span></div>
          <div class="detail-item"><label>审核状态</label>
            <span :class="['status-tag', getAuditClass(detailItem.auditStatus)]">{{ getAuditText(detailItem.auditStatus) }}</span>
          </div>
          <div class="detail-item full-width" v-if="detailItem.auditRemark"><label>审核备注</label><span>{{ detailItem.auditRemark }}</span></div>
        </div>
        <div class="detail-images">
          <div class="img-block">
            <label>身份证正面</label>
            <img v-if="detailItem.idCardFront" :src="detailItem.idCardFront" class="id-big-img" @click="previewImage(detailItem.idCardFront)" />
            <span v-else class="no-img">-</span>
          </div>
          <div class="img-block">
            <label>身份证反面</label>
            <img v-if="detailItem.idCardBack" :src="detailItem.idCardBack" class="id-big-img" @click="previewImage(detailItem.idCardBack)" />
            <span v-else class="no-img">-</span>
          </div>
        </div>
        <div class="dialog-btns">
          <template v-if="detailItem.auditStatus === 0">
            <button class="btn-approve" @click="audit(detailItem, 1)">通过</button>
            <button class="btn-reject" @click="openReject(detailItem)">拒绝</button>
          </template>
    ...
    <div v-if="previewVisible" class="modal-mask" @click.self="previewVisible = false">
      <div class="preview-box">
        <span class="close-preview" @click="previewVisible = false">&times;</span>
...
const getAuditClass = (status: number) => {
  if (status === 0) return 'WAIT'; if (status === 1) return 'PASSED'; if (status === 2) return 'REJECTED'; return 'UNKNOWN'
}
const getAuditText = (status: number) => {
  if (status === 0) return '待审核'; if (status === 1) return '已通过'; if (status === 2) return '已拒绝'; return '未知'
}
...
const previewImage = (url: string) => { previewUrl.value = url; previewVisible.value = true }
...
const loadData = async () => {
  ...
  } catch (e: any) { console.error('[分销员审核] 加载失败:', e.message) }
}

const openDetail = (item: any) => { detailItem.value = item; detailVisible.value = true }

const audit = async (item: any, auditStatus: number) => {
  try {
    const res = await request.post(`/admin/distributor/applies/${item.id}/audit`, { auditStatus, auditRemark: '' })
    if (res.code === 0) {
      detailVisible.value = false
      loadData()
    } else { alert(res.message || '操作失败') }
  } catch (e: any) { alert('操作失败: ' + (e.message || '网络错误')) }
}

const openReject = (item: any) => { currentItem.value = item; rejectRemark.value = ''; showReject.value = true }
const doReject = async () => {
  if (!currentItem.value || !rejectRemark.value.trim()) { alert('请输入拒绝理由'); return }
  await request.post(`/admin/distributor/applies/${currentItem.value.id}/audit`, { auditStatus: 2, auditRemark: rejectRemark.value })
  showReject.value = false; detailVisible.value = false; loadData()
}
...
.data-table { width: 100%; border-collapse: collapse; background: #fff; border-radius: 6px; overflow: hidden; white-space: nowrap; min-width: 900px; }
...
.btn-detail { padding: 4px 10px; background: #F2F3F5; color: #4E5969; border: none; border-radius: 4px; cursor: pointer; font-size: 12px; margin-right: 6px; }
...
.remark-text { color: #999; font-size: 12px; margin-left: 6px; }
...
.modal-box { background: #fff; border-radius: 8px; padding: 24px; width: 480px; max-height: 90vh; overflow-y: auto; }
...
.detail-images { display: flex; gap: 24px; margin-bottom: 16px; }
.img-block { display: flex; flex-direction: column; align-items: center; }
.img-block label { font-size: 12px; color: #999; margin-bottom: 6px; }
.id-big-img { width: 240px; height: 150px; object-fit: cover; border-radius: 6px; border: 1px solid #e8e8e8; cursor: pointer; }
